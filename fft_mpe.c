...
int event_sendrecv_begin, event_sendrecv_end;
int event_barrier_begin, event_barrier_end;
int event_calc_begin, event_calc_end;
...
void fft_compute_stages(double complex *local_x, int N, int local_N, int rank, int size) {
    if (rank != 0) {
        MPE_Log_event(event_calc_begin, 0, NULL);
    }
    int stages = 0;
    while ((1 << stages) < N) {
        stages++;
        ...
        for (int k = 0; k < N; k += m) {
            ...
            for (int j = 0; j < m2; j++) {
                ...
                if (rank == rank1 && rank == rank2) {
                    ...
                    MPE_Log_event(event_sendrecv_begin, 0, NULL);
                    MPE_Log_send(rank2, 0, 1);
                    MPI_Sendrecv(&local_x[local_idx1], 1, MPI_DOUBLE_COMPLEX, rank2, 0,
                                 &val_idx2,          1, MPI_DOUBLE_COMPLEX, rank2, 0,
                                 MPI_COMM_WORLD, &status);
                    MPE_Log_receive(rank2, 0, 1);
                    MPE_Log_event(event_sendrecv_end, 0, NULL);
         ...
         MPE_Log_event(event_barrier_begin, 0, NULL);
         MPI_Barrier(MPI_COMM_WORLD);
         MPE_Log_event(event_barrier_end, 0, NULL);
    }
    if (rank != 0) MPE_Log_event(event_calc_end, 0, NULL);
}
...
int main(int argc, char *argv[]) {
    ...
    MPE_Init_log();
    MPE_Start_log();
    MPE_Log_get_state_eventIDs(&event_sendrecv_begin, &event_sendrecv_end);
    MPE_Describe_state(event_sendrecv_begin, event_sendrecv_end, "MPI_Sendrecv", "blue");

    MPE_Log_get_state_eventIDs(&event_barrier_begin, &event_barrier_end);
    MPE_Describe_state(event_barrier_begin, event_barrier_end, "MPI_Barrier", "green");

    MPE_Log_get_state_eventIDs(&event_calc_begin, &event_calc_end);
    MPE_Describe_state(event_calc_begin, event_calc_end, "Computing", "red");

    MPE_Log_get_known_eventID();
    MPE_Log_get_known_stateID();
